...
| T22 | **Body measure is ~39 characters at 360px, just under our own 40 floor.** Found while sizing `T21`, and it is not a type-size problem: measured in the preview, body at 16px averages 8.38px per character, so 360px minus the 16px `--grid-margin-sm` on each side gives 39 characters per line. Our preview states "below 40 characters the eye returns too often". Two floors collide here — 16px is the minimum for UI body text, 40ch the minimum measure — and on the narrowest phones both cannot hold. Shrinking body would raise the character count and breach the size floor, so **margin is the only free variable**: `--grid-margin-sm` at 12px yields 40 characters. Left open because narrowing the gutter affects every mode's layout at small widths, which is a decision rather than a correction. | `--grid-margin-sm` in `mode.*.css` | | ⬜ |
| --- | --- | --- | --- | --- |
