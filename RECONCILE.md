...
| # | Current default | Where | Status |
| --- | --- | --- | --- |
| T9 | **Scale ratio varies by mode** 1.250 / 1.200 / 1.125 — reverses the earlier unification | `mode.*.css` | ✅ |
| T10 | `--text-prose` (18px) separated from `--text-body` (UI text) | `mode.*.css`, `B-75` | ✅ |
| T11 | **Two weights only**, regular + bold — `--font-weight-medium` removed | `mode.*.css`, `B-77` | ✅ |
| T12 | Measure 40–80 characters, stated as the rule | `02-tokens.md` | ✅ |
| T13 | Line height 1.5–2 for prose; raise for long lines / heavy faces | `02-tokens.md` | ✅ |
| T14 | One sans serif; second face for headings only | `B-76` | ✅ |
| T15 | Justified text prohibited outright | `B-12` | ✅ |
| T16 | Four treatments for text on photos | `B-78` | ✅ |
| T17 | Letter spacing tightens as size grows | `mode.*.css` | ✅ |
| --- | --- | --- | --- |
| T1 | Body **16 / 16 / 14px**. Editorial dropped 18→16 | `mode.*.css` | ✅ |
| T2 | **One scale, ratio 1.200**, all modes: 14/16/20/24/32/40 | `mode.*.css` | ✅ |
| T3 | Line height 1.65 body → 1.05 display. **Floor 1.5 on body/secondary, all modes** | `mode.*.css` | ✅ |
| T4 | Measure 68 / 60 / 72ch | `mode.*.css` | ⬜ |
| T7 | Line height = size + 8, on 4pt grid | `mode.*.css` | ✅ |
| T8 | **Line heights unitless, not px.** 1.5 floor on body/secondary | `mode.*.css` | ✅ |
| T5 | Weight 400 body minimum, 600 headings | `mode.*.css`, `B-14` | ⬜ |
| T6 | Negative tracking on headings only | `mode.*.css` | ⬜ |
...
| F2 | **Three radii: 8 / 16 / 32px** by element size | `brand.default.css` | ✅ |
