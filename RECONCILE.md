...
| T19 | **Line height decreases as size increases** (p.247), and long body sits between 1.5 and 2 (p.245). Ours holds in every mode, but this row originally quoted `product`'s numbers as if universal and `02-tokens.md` repeated the same mistake in prose. Leading is per-mode: heading leadings are one ladder (1.1 · 1.25 · 1.333 · 1.4 · 1.5) read through a three-rung sliding window that starts lower the denser the mode — `editorial` 1.1/1.25/1.333, `product` 1.25/1.333/1.4, `operator` 1.333/1.4/1.5 — which is why `product`'s H1 leading equals `editorial`'s H2. Now a table in `02-tokens.md`, checked by Rule 7, with Rule 8 asserting the shape itself: leading never increases as size does, body floors at 1.5, prose stays in the 1.5–2 band. | `mode.*.css`, `02-tokens.md` |  | ✅ |
...
| T21 | **Responsive type scales — adopted, as fluid headings.** The reference recommends dropping to a smaller scale on mobile to avoid wrapping; we had no mechanism at all, every size fixed at every width. `--text-h1` and `--text-h2` in `editorial` and `product` are now `clamp()`, interpolating between a 360px viewport and 1024px. Chosen over a second `-sm` scale because a clamp has no threshold to place, and this system defines no breakpoint token to place one at. Measured before and after in the preview: editorial `--text-h1` at a fixed 48px gave 13 characters per line at 360px, so a 45-character headline set as four lines and 211px; at the 32px minimum it is three lines and 106px. `operator` is deliberately unchanged — its largest heading is 22px and already fits ~28 characters. Body and prose stay fixed at every width: readability is absolute, hierarchy is relative, and a fluid `--text-prose` would breach the 18px floor `T20` just raised. | `mode.editorial.css`, `mode.product.css` | `02-tokens.md` | ✅ |
| T22 | **Measure floor kept as it is — 40 characters is unreachable on a small phone, not narrowly missed.** The proposed fix was `--grid-margin-sm` 16px → 12px to buy the one character. Measured across real widths instead of arguing it: at 320px body sets 34 characters with the current gutter, 35 with a 12px one, and **38 with no gutter at all**. Forty characters of 16px text need ~335px of width before any margin exists, so below ~367px the target and the 16px body floor are geometrically incompatible. The body floor is the one that cannot move — `B-75`, `T20`, WCAG 1.4.4, and both sources consulted say never below 16px on any screen — so the measure target yields. At 375px, the modern small-phone baseline and one of the widths the source names for testing, the target is already met at 41 characters with the gutter untouched. Narrowing it would cost layout at every small width, buy one character at 360px and change nothing at 320px. `02-tokens.md` now carries the table and says outright not to chase the number. | `--grid-margin-sm` in `mode.*.css` | `02-tokens.md` | ➖ |
| --- | --- | --- | --- | --- |
| T1 | Body **16 / 16 / 14px**. Editorial dropped 18→16 | `mode.*.css` | `01-modes.md` | ✅ |
| T2 | ~~One scale, ratio 1.200, all modes: 14/16/20/24/32/40.~~ **Superseded by `T18`**, which established a per-mode ratio (1.250 / 1.200 / 1.125) because scale size should track interface complexity. No mode has a 40px type size any more. Kept as a record of the decision this one replaced; the row was left marked ✅ describing a state that had stopped existing, which is worse than an open row because it reads as verified. | `mode.*.css` | `01-modes.md` | ✅ |
| T3 | ~~Line height 1.65 body → 1.05 display.~~ **Superseded by `T19`.** No mode has 1.65 or 1.05; the range is now 1.6 prose down to 1.1 on editorial H1. The surviving half of this row is the **1.5 floor on body and caption**, which still holds in all three modes and is now enforced by `check-tokens` rule 8. | `mode.*.css` | `01-modes.md` | ✅ |
...
| T7 | ~~Line height = size + 8, on a 4pt grid.~~ **Superseded by `T19`, and it was marked ✅ against tokens that never satisfied it.** Checked against editorial: it holds for body (24), h3 (32) and h2 (40), and fails for caption (21 vs 22), prose (28.8 vs 26), lead (30 vs 28) and h1 (52.8 vs 56) — three of seven. Line heights are unitless ratios chosen per tier, not an additive rule, which is why they scale correctly when `--text-h1` became fluid and an additive rule would not have. | `mode.*.css` | `01-modes.md` | ✅ |
...
| F11 | **Shadow colour stays black, not the text-strong palette variation.** The reference suggests tinting shadows with the foreground colour so they sit with the rest of the interface. In this system that instruction resolves to the same value it already has: foregrounds here are *achromatic opacities* by construction — `--color-text-strong` is `rgb(0 0 0 / 90%)`, not a hue — precisely so they read correctly on any surface (`C-21`). Adopting the reference's phrasing would imply a hue-tinted shadow, which is a different decision than the one it is making. The counter-argument is real and recorded rather than dismissed: `--color-bg-base` is a warm off-white, and a pure-black shadow on it reads very slightly cold. Not enough to introduce a brand-hue dependency into elevation, which is also the layer this system already prefers not to use (`A-08`, stroke-led). | `brand.default.css` | `A-08` | ➖ |
| F12 | **Disabled opacity stays 0.38 against the reference's 20%, and the reference's own table is why.** This divergence had been recorded for two releases without anyone computing it. APCA, at 16px on white: opacity 0.20 gives **Lc 27.3**, below the **Lc 30** the reference's own APCA table sets as the absolute minimum for disabled button text; 0.38 gives **Lc 52.1**, clearing that and clearing Lc 45 for interface elements. Lc 30 is not reached until 0.218. The reference holds two positions that contradict each other and only one has a number in it. Note WCAG 2.1 exempts disabled controls entirely, so APCA is the *only* standard constraining this value — the case `02-tokens.md` describes for checking both. Now enforced by `check-tokens` rule 11, which rejects 0.20 by name. | `brand.default.css` | `02-tokens.md`, `P-02` | ➖ |
| F13 | **APCA is now referenced where the contrast rules live.** `02-tokens.md` carried the threshold table; `00-anti-patterns.md`, which is the file an agent actually reads while writing colour, said nothing about it — so every ratio there read as the whole picture. Section C now states that its ratios are WCAG 2.1 by policy, points at the table, and names the two places the systems diverge: WCAG's exemption of disabled controls, where APCA is the only constraint, and the fact that the two disagree about what counts as large text. **No APCA detector was added, deliberately.** `check` fails builds on WCAG 2.1 AA because that is what is legally referenced; a mechanical rule failing a build against a draft standard would assert more than the draft does. The algorithm ships as `scripts/apca.mjs` and holds the token set's own values to it. | `00-anti-patterns.md` | `02-tokens.md` | ✅ |
...
### Sources for the responsive rows

`T21` and `T22` were settled against two responsive-design references rather than the primary book, which predates the question.

A responsive-typography guide (<https://subux.pro/guides/article/responsive-design-typography>) confirms the whole `T21` approach independently and
in the same order: define type in `rem` never fixed `px`, pair with `clamp()` for fluid typography, scale headings on mobile but hold body at 1rem
across every breakpoint, and never below 16px because `px`-only "breaks user zoom preferences". That is the same WCAG 1.4.4 argument we proved
empirically before reading it. Two differences worth recording: it suggests reducing headings ~20–25% on mobile where our `editorial` drops 33%
(48→32), which is warranted by a larger starting size — `product` at 25% (32→24) sits inside its range; and it recommends raising line height to 1.5
on small screens, which we already satisfy everywhere since `--leading-body` is 1.5 in all three modes. Its test widths — 320, 375, 768, 1024 —
added the 320px case that settled `T22`; our own testing had started at 360 and would have missed the width where the floor becomes unreachable.

Figma's responsive-design resource (<https://www.figma.com/resource-library/responsive-website-design/>) is an introduction to the CSS mechanics —
flexbox, grid, media queries, responsive navigation and images. It endorses `clamp()` for fluid type and contradicts nothing, but states no value we
could adopt and says nothing about measure or gutters. Recorded as checked, not as a source.
...
| M5 | **Ambient durations are tokenised** — `--duration-ambient-fast/base/slow` (3s / 4.7s / 7.1s), `editorial` only. `P-13` first argued against tokenising them, on the grounds that layered ambient motion needs its parts to have *different* periods and one shared token would synchronise them. That was an argument against a single token, not against tokens, and it was overturned: a set of three keeps the layering and removes the raw seconds that `H-47` forbids everywhere else. The values are mutually prime in tenths (30 / 47 / 71), so all three first re-align past two hours — 3 / 4 / 6s would re-align every twelve seconds. They are deliberately absent from the reduced-motion 1ms override, because 1ms on an infinite loop is a thousand cycles a second rather than stillness. | `mode.editorial.css`, `P-13` | `02-tokens.md` | ✅ |
