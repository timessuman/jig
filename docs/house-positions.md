...
- `P-14` a word on the menu button, and no sideways-scrolling nav row. Contested — an icon-only menu is the default almost everywhere, and a scrolling tab row is common on phones. The word is kept because an icon asks the reader to already know the convention, and the scroll is refused because its last items go past the edge where nobody looks. Phone-first composition is the premise: the narrow control is designed, not derived.
