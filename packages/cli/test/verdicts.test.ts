...
describe('jig verdicts', () => {
  it('passes a complete review and computes the counts itself', () => {
    ...
    for (const width of [360, 768, 1280, 1600]) {
      saveProbe({ projectRoot: project, surface: 'pricing', json: JSON.stringify({ jigProbe: 5, url: `file://${join(project, 'pricing.html')}`, width, sidewaysScroll: false, scrollWidth: width, clientWidth: width, defaultFont: false, unresolvedTokens: [], junkText: [], brokenImages: 0, navLinksVisible: 5, menu: null }) });
    }
    const r = run();
