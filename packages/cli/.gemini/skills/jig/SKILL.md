...
1. Load `.gemini/skills/jig/rules/00-anti-patterns.md` and `.gemini/skills/jig/rules/01-modes.md`.
2. Determine the mode from `jig.config.json`, or infer it using the procedure in
   `.gemini/skills/jig/rules/01-modes.md` and state the inference in one line before building.
3. Load the relevant section of `.gemini/skills/jig/rules/03-patterns.md` for the component you are building.
4. Load `.gemini/skills/jig/rules/05-copy.md` whenever you write a label, button, heading, error or empty state.
5. Consume tokens by semantic name only. Never write a raw colour or pixel value
   at a call site.
6. Run the self-check at the end of `.gemini/skills/jig/rules/00-anti-patterns.md` before finishing
   (a future `check` command will automate this).
7. Cite any rule you deliberately break, with the reason, in one line.

Load `.gemini/skills/jig/rules/04-principles.md` only when two rules conflict.
